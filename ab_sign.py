...
抖音 webcast 接口的 a_bogus 签名算法（SM3 + RC4），纯 Python，无 Node.js 依赖。
改编自 MIT 项目 ihmily/DouyinLiveRecorder 的 src/ab_sign.py，Copyright 2025 Hmily（保留版权声明）。

对外接口（保持与占位版一致，resolver.py 无需改调用方式）：
    get_a_bogus(url: str, params: dict, data: dict = None, user_agent: str = "") -> str
    is_available() -> bool

低层函数（与上游同名，便于对照/测试）：
    ab_sign(url_search_params: str, user_agent: str) -> str
"""
...
def get_a_bogus(url: str, params: dict, data: dict = None, user_agent: str = "") -> str:
    """生成 a_bogus 签名。

    url: 接口 base（如 https://live.douyin.com/webcast/room/web/enter/）
    params: query 参数 dict（不含 a_bogus 本身）
    data: 预留，未使用
    user_agent: 签名用的 UA，需与请求头一致
    """
    ua = user_agent or DEFAULT_UA
    query = urllib.parse.urlencode(params)
    full = url + ("&" if "?" in url else "?") + query
    return ab_sign(urllib.parse.urlparse(full).query, ua)
...
if __name__ == "__main__":
    import sys
    q = sys.argv[1] if len(sys.argv) > 1 else "aid=6383&app_name=douyin_web&live_id=1&device_platform=web&language=zh-CN&web_rid=335354047186&msToken="
    ua = sys.argv[2] if len(sys.argv) > 2 else DEFAULT_UA
    s = ab_sign(q, ua)
    print(f"len={len(s)}")
    print(s)
